...
and populates todo.work_actions, using AI classification to handle duplicates
and progressions.
...
PROMPT_TEMPLATE = """You are classifying candidate work actions extracted from a meeting summary. C tracks work obligations in a todo list. Your job: determine whether each candidate is genuinely new, an update to an existing item, or a restatement of something already tracked.

## OBLIGATION IDENTITY TEST

Before classifying any candidate as PROGRESSION or DUPLICATE, apply this test.

Primary gate — same deliverable? Ask: does the candidate produce the same concrete output as the existing item? If both were completed, would they produce one thing or two things? This is a semantic comparison, not a string match — "prepare briefing for Ben" and "pull together a one-pager for Ben" are the same deliverable. But "draft a reply to Dave" and "get Ange's input on wording before replying to Dave" are different deliverables — one produces a draft, the other produces alignment.

If the deliverables are different, the candidate is NEW. Stop. No further analysis needed.

Apply the deliverable test at the concrete action level, not the project or thread level. Two actions may serve the same broader goal but produce different outputs. Compare what each action produces, not what objective it serves.

Shared people, shared topic, shared meeting thread, and shared conversational context do not establish obligation identity. These are correlation signals, not identity signals. The deliverable test is the only valid basis for a match.

If the deliverable is the same, use secondary signals to distinguish PROGRESSION from DUPLICATE:
- Dependency resolved, added, or changed = PROGRESSION
- Date added or changed = PROGRESSION
- Method or approach refined toward the same output = PROGRESSION
- Clear status/state update about the same output = PROGRESSION
- No change on any dimension = DUPLICATE

If the action changes the required output itself, the deliverable has changed. Classify as NEW. Examples: "draft reply" vs "send reply"; "explain" vs "get agreement". A pure status update about the same output is not a deliverable change. Examples: "send draft to Ben" vs "draft sent to Ben"; "book room" vs "room booked" = PROGRESSION.

## EXISTING ACTIONS

Each line shows: [UID] [status] summary | Due: date | Source: meeting, date
Some items include a "Last progression" line. Use progression notes only to disambiguate which specific instance an item refers to (e.g. "April pack" vs the general monthly pack). Do not use progression notes to widen the match surface to topics, people, or actions not in the summary text.
...
NEW — The deliverable test finds no match in existing actions. This is a genuinely new obligation.

PROGRESSION — The deliverable test confirms the same concrete output as an existing active item, but the candidate carries forward movement: a new date, a refined method, a resolved dependency, or a status/state update for that same output. Substitution test: if the candidate were completed instead of the existing item, would the same concrete output be produced? If yes, this is PROGRESSION.

Hard exclusions — these are always NEW, never PROGRESSION:
- Prerequisite actions (get approval before sending) — different deliverable
- Downstream actions (send after drafting) — different deliverable
- Sibling actions in the same thread but with different outputs — different deliverable

DUPLICATE — The deliverable test confirms the same output as an existing item, and the candidate adds no new information. No date change, no method refinement, no dependency change, no status/state update.

## EXAMPLES

These three candidates all involve the same topic and people. The verdicts differ because the deliverables differ.

Existing item: [UID-A] [needs_action] Send draft to Ben | Due: no date
Existing item: [UID-B] [needs_action] Explain policy to Dave | Due: no date

Example 1 — NEW:
Candidate: "Get Ange's input on wording before replying to Dave"
Verdict: NEW (matched_uid: null)
Reason: Different deliverable from UID-B. Explaining a policy produces an explanation to Dave. Getting input produces alignment with Ange. Completing one does not produce the same output as the other.

Example 2 — PROGRESSION:
Candidate: "Send draft to Ben by Friday"
Verdict: PROGRESSION (matched_uid: UID-A)
Reason: Same deliverable as UID-A. Date added. Completing the candidate produces the same output with a deadline attached.

Example 3 — DUPLICATE:
Candidate: "Send the draft to Ben"
Verdict: DUPLICATE (matched_uid: UID-A)
Reason: Same deliverable as UID-A, no new information. No date, no method change, no dependency change, no status update.

## SITUATIONAL RULES

Date changes: a date change is PROGRESSION only when the deliverable test confirms the same obligation. A different date referencing a different event, time period, or instance is NEW. Example: "Book room for June quarterly" and "Book room for September quarterly" are different instances — NEW.

Completed items: a candidate matching a completed item is DUPLICATE only if it refers to the same specific instance — same event, same time period, same document version. A new instance of a recurring obligation is NEW regardless of wording similarity. Apply the substitution test: would completing the candidate re-produce the output that was already completed?

Tie-breaking: if a candidate could match more than one existing item, prefer the closest instance match (same event, same period, same deliverable instance) regardless of active/completed status. If instance evidence is equal, prefer the active item. If genuinely ambiguous, classify as NEW.

## ANTI-COLLISION

If two or more candidates would map to the same existing UID, pause. Apply the deliverable test between the candidates themselves. If the candidates produce different outputs, at most one matches that existing item. Re-evaluate the others against the full existing list — they may match a different existing item. Only classify as NEW if no match exists anywhere in the list.

If the candidates share a deliverable pattern but refer to different instances (e.g. "February KPI pack" and "March KPI pack"), they are separate obligations. Evaluate each independently.

Each candidate is an independent assessment. Topical similarity between candidates does not mean one must be a duplicate or progression of the other.

## SAFETY BIAS

- Uncertain between NEW and DUPLICATE? Classify NEW. A duplicate C manually removes costs seconds. A missed action costs professional credibility.
- Uncertain between PROGRESSION and DUPLICATE? Classify PROGRESSION. An unnecessary update is harmless. A missed update means stale information.
- Never classify a genuinely new action as DUPLICATE. When in doubt, add.

## OUTPUT FORMAT

The "verdicts" field must contain only a valid JSON array. No markdown fences, no commentary, no text outside the array. The array must contain exactly one verdict per candidate, covering every index from 0 to N-1. Do not skip candidates. Do not duplicate indices. Each element:

- "candidate_index": integer (0-based, matching candidate list order)
- "verdict": "NEW" or "PROGRESSION" or "DUPLICATE"
- "matched_uid": for NEW, must be null. For PROGRESSION and DUPLICATE, must be non-null — copy a UID character-for-character from the [UID] shown in the existing actions list. Do not shorten, modify, or invent UIDs.
- "reason": one sentence explaining the classification

For PROGRESSION only, optionally include:
- "updated_due_date": "YYYY-MM-DD" if the candidate has a different/new date, else null
- "updated_text": the candidate's action text ONLY if it describes the same deliverable with more precise or current language. Do not use updated_text to replace one obligation with another. If the verdict is PROGRESSION, the deliverable test already passed, so the question is whether the candidate's wording is a better description of that same deliverable. When in doubt, null — a stale item is visible, a corrupted item is silent damage.

The "verdicts" field must contain ONLY the JSON array."""
...
def extract_actions(description, summary_field):
    ...
    # Work classification gate - skip non-work recordings
    if re.search(r'^- Work:\s*no\s*$', text, re.MULTILINE | re.IGNORECASE):
        return [], None, None, False
    ...
    for line in lines:
        ...
        if not in_section:
            # Match any heading level for "Actions"
            if re.match(r'^#+\s+Actions', stripped):
                in_section = True
            continue
...
def fetch_existing_actions(now):
    """Fetch all items from todo.work_actions (active + recent completed).
...
def resolve_collisions(verdicts):
    ...
    candidate_index reflects extraction order (top-to-bottom in ## Actions).
        ...
        if verdict == "NEW":
            ...
            try:
                ...
                if manual_edit:
                    ...
                    log.info(
                        f"Action pipeline: manual edit preserved — "
                        f"Item: {current_summary[:60]} | "
                        f"Progression context appended but summary/due not overwritten. "
...
def report_dry_run(source_uid, meeting_title, meeting_date, candidates,
    ...
    log.info(f"Action pipeline: DRY RUN — {chr(10).join(lines)}")
...
def reacquire_expired_lease(summary_uid, meeting_date, meeting_title, now):
    ...
    try:
        ...
        log.info(
            f"Action pipeline: stale lease recovered — "
            f"Summary: {summary_uid[:8]} | "
            f"Orphaned 'processing' lease recovered after TTL expiry. Re-processing."
        )
        return True
    except Exception as e:
        notify(
